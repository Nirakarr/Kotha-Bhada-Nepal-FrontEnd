...
const Card = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        // Sort posts by updatedDate in descending order
        const sortedPosts = response.data.data.sort((a, b) => {
          return (
            new Date(b.updatedDate || b.date) -
            new Date(a.updatedDate || a.date)
          );
        });

        // Limit the results to the latest 12 posts
  ...
  return (
    <>
      <div className="grid gap-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 p-0 m-auto">
        {posts.map((item, key) => (
          <Link
